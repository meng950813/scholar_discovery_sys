...
class ChinaMap{
    constructor(svg){
        ...
        this.visible = false;
        //要加载的文件路径(也是视图函数的路径)
        this.filepath_list = ['api/mapdata/china.json', 'api/mapdata/geometryProvince', 'api/mapdata/geometryCouties'];
        //栈，用于保存文件名和地图名
    ...
    /**
     * 公有函数 通过列表项来点击地图块，若列表项没有对应的地图块则不做任何操作，并返回false。
     * @param item 列表数据项
     * @returns {boolean} 如果点击地图块成功则返回true，否则返回false
     */
    clickMapBlockByItem(item){
    ...
    /**
     * 外部重写函数 以texts来更新对应的面包屑导航栏
     * @param texts {Array} 更新导航栏的显示文本
     */
    onUpdateBreadCrumb(texts){
    }
    ...
    redrawMap(jsondata){
        ...
        //回调更新地图钩子函数
        if (items.length > 0){
            this.updateMapCallback(items.sort(function (a1, a2) {
    ...
    clickBreadcrumb(index) {
        ...
        //维护栈
        this.stack.splice(this.scaleLevel,  this.stack.length - this.scaleLevel);
